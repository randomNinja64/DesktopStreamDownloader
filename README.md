...
To build the release version of the project, place the required executables (`curl.exe`, `curl-ca-bundle.crt`, `yt-dlp.exe`, and `ffmpeg.exe`) in the `deps` folder and use the `build.bat` script (Visual Studio/Visual Studio Build Tools/MSBuild required).
